...
export function TransactionDialog({ children, defaultOpenOcr = false, open: controlledOpen, onOpenChange: setControlledOpen }: { children: React.ReactNode, defaultOpenOcr?: boolean, open?: boolean, onOpenChange?: (open: boolean) => void }) {
    ...
    // Update payment method when account changes (for simple mode)
    // Only for rows that are "CREDIT" for Assets or "DEBIT" for Liabilities (depending on logic)
    // Actually, usually user selects "Paid By" account.
    // If we have simple rows, it's ambiguous.
    // BUT, the user requested "Payment time should be added on transaction".
    // And "Split would be on the individual card".

    // Let's add a "Payment Method" selector next to the Account selector in Rows if applicable?
    // OR, better, if a row selects an account with payment methods, show the selector.
    ...
    // Update row function extended
    const updateRow = (index: number, field: keyof JournalEntryRow | 'paymentMethodId', value: string) => {
        const newRows = [...rows]
        if (field === 'paymentMethodId') {
            // We store paymentMethodId at transaction level or row level?
            // Schema has `payment_method_id` on `transactions` table.
            // This implies the WHOLE transaction is linked to one card.
            // This works for simple "I bought coffee" (1 transaction).
            // But if user splits... "I bought coffee and a muffin".
            // It's still one card swipe.
            // So `paymentMethodId` should be a top-level state if we assume one card per transaction.
            // However, `rows` defines the accounts.
            // Usually one of the rows is the "Source" (Asset/Liability).
            // If that source account has payment methods, we should pick one.

            // Let's try to derive/set the top-level paymentMethodId based on the "Source" account selected in rows.
            // But rows can be mixed.
            // Let's add a dedicated "Payment Details" section if a relevant account is involved?
            // Or just put it under the description?

            // Re-reading user request: "for each account there could be multiple cards... this split would be on the individual card."
            // "Card is not mandatory. there may be times it is cash."

            // If I stick to `payment_method_id` on `transactions`, I can only support ONE card per transaction.
            // This is standard. You don't pay 50% with Visa and 50% with Mastercard in the SAME transaction receipt usually.
            // If you do, it's two transactions or a split payment which is complex.
            // Let's assume 1 transaction = 1 payment method.

            // So, UI:
            // When user selects an account in rows that has payment methods, we should offer to select one.
            // Since we can have multiple rows, which one controls the payment method?
            // Usually the CREDIT on Asset or CREDIT on Liability (spending money).
            // Let's add a global "Payment Method" selector that filters based on selected accounts?
            // Or just show it if any selected account has methods.
        ...
        // Auto-select first payment method if account changes and has them?
        if (field === 'accountId') {
            const pms = getAccountPaymentMethods(value)
            if (pms.length > 0) {
                // Set globally?
                // If we have multiple accounts with PMs, this might be tricky.
                // Let's simplify: Show a "Payment Method" dropdown below the Rows if valid.
                setPaymentMethodId(null) // Reset on account change to force selection or default
            }
        }
    ...
    const handleSubmit = async () => {
        ...
        if (lineItems.length > 0) {
            ...
            const batch = lineItems.map((li, index) => ({
                ...
                paymentMethodId: index === 0 ? (paymentMethodId || undefined) : undefined // Only link to first or all? 
                // Any logic: If they are splitting a receipt, they likely used the same card.
                // So pass it to all.
            })).map(b => ({ ...b, paymentMethodId: paymentMethodId || undefined })) // Apply to all
    ...
    // We look for accounts in rows (or bankAccountId in scan mode) that have payment methods.
    ...
    // Flatten all PMs from these accounts
    const availablePaymentMethods = relevantAccounts.flatMap(a => a.payment_methods || [])

    // Deduplicate (though ID should be unique)
    const uniquePaymentMethods = Array.from(new Set(availablePaymentMethods.map(pm => pm.id)))
        .map(id => availablePaymentMethods.find(pm => pm.id === id)!)
